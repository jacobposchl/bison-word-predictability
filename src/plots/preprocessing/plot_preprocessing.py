...
def plot_switch_position(df: pd.DataFrame, figures_dir: str) -> None:
    """
    Plot distribution of code-switch positions in sentences.
    ...
    # Calculate sentence lengths and switch positions
    df = df.copy()
    ...
    # Subplot 1: Histogram of switch positions
    ax1 = axes[0]
    for group in groups:
        group_df = df[df['group'] == group]
        if len(group_df) > 0:
            ax1.hist(group_df['switch_position'], bins=20, alpha=0.6, label=group, density=True)
    
    ax1.set_xlabel('Normalized Switch Position (0 = start, 1 = end)', fontsize=12)
    ax1.set_ylabel('Density', fontsize=12)
    ax1.set_title('Distribution of Code-Switch Positions', fontsize=14, fontweight='bold')
    ax1.legend()
    ax1.grid(axis='y', alpha=0.3)
    ax1.axvline(0.5, color='black', linestyle='--', alpha=0.5, label='Midpoint')
    
    # Subplot 2: Box plot by group
    ax2 = axes[1]
    data_for_box = [df[df['group'] == group]['switch_position'].values for group in groups if len(df[df['group'] == group]) > 0]
    labels_for_box = [group for group in groups if len(df[df['group'] == group]) > 0]
    ...
    ax2.set_ylabel('Normalized Switch Position', fontsize=12)
    ax2.set_xlabel('Speaker Group', fontsize=12)
    ax2.set_title('Switch Position by Speaker Group', fontsize=14, fontweight='bold')
    ax2.grid(axis='y', alpha=0.3)
...
def plot_sentence_length_distribution(df: pd.DataFrame, figures_dir: str) -> None:
    ...
    # Subplot 1: Violin plot by group
    ax1 = axes[0]
    ...
    parts = ax1.violinplot(data_for_violin, positions=range(len(labels_for_violin)), 
    ...
    ax1.set_xlabel('Speaker Group', fontsize=12)
    ax1.set_title('Sentence Length Distribution by Speaker Group', fontsize=14, fontweight='bold')
    ax1.grid(axis='y', alpha=0.3)
    
    # Subplot 2: Histogram by matrix language
    ax2 = axes[1]
    for matrix_lang in ['Cantonese', 'English', 'Equal']:
        lang_df = df[df['matrix_language'] == matrix_lang]
        if len(lang_df) > 0:
            ax2.hist(lang_df['sentence_length'], bins=30, alpha=0.6, label=matrix_lang, density=True)
    
    ax2.set_xlabel('Sentence Length (words)', fontsize=12)
    ax2.set_ylabel('Density', fontsize=12)
    ax2.set_title('Sentence Length by Matrix Language', fontsize=14, fontweight='bold')
    ax2.legend()
    ax2.grid(axis='y', alpha=0.3)
...
def plot_participant_variation(df: pd.DataFrame, figures_dir: str) -> None:
    ...
    ax.set_ylabel('Number of Code-Switched Sentences', fontsize=12)
    ax.set_xlabel('Speaker Group', fontsize=12)
    ax.set_title('Participant-Level Variation in Code-Switching', fontsize=14, fontweight='bold')
    ax.grid(axis='y', alpha=0.3)
